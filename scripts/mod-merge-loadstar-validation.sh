...
if [[ ! -f "$DEFAULT_PC_PAK" && -f "fixtures/pc.pak" ]]; then
  DEFAULT_PC_PAK="fixtures/pc.pak"
fi
...
for path in "$BASE_PAK" "$MOD_PAK" "$PC_PAK"; do
  if [[ ! -f "$path" ]]; then
...
3. Launch SnowRunner and check the Loadstar JBE truck, wheels, meshes, textures, and UI images.
4. Record the result in $REPORT or docs/mod-merge-notes.md.
EOF
